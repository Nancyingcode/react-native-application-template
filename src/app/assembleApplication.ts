...
export function assembleApplication(
  ...
  const moduleOrder = new Map(brand.assembly.modules.map((id, index) => [id, index]));
  ...
  const menus = selected
    ...
    .filter(item => brand.assembly.menu.includes(item.id) && assertRoute(item.route))
    .sort(
      (a, b) =>
  ...
  const home = selected
    ...
    .filter(item => isContributionVisible(brand.features, runtime, item))
    .filter(item => brand.assembly.home.includes(item.id) && assertRoute(item.route))
    .sort(
      (a, b) =>
  ...
  const initialRoute = brand.assembly.initialRoute === 'Home'
    ? 'Home'
    : assertRoute(brand.assembly.initialRoute)
    ? brand.assembly.initialRoute
    : routes[0]?.name;
  if (!initialRoute) {
